...
def q1_time(file_path: str) -> List[Tuple[datetime.date, str]]:
    ...
    cantidad de tweets, para cada una de estas fechas entrega el usuario con 
    mayor cantidad de tweets, se prioriza el tiempo de ejecucion.
    ...
    df = df.with_columns([pl.col('date').str.strptime(
        pl.Date, "%Y-%m-%dT%H:%M:%S%z").alias('date')])
    ...
    result = (df_filtrado.group_by(['date', 'username'])
              ...
              # Se ordena por fecha desendente false y largo true
              .sort(by=['date', 'len'], descending=[False, True])
    ...
    # Se formatea el output para que sea consistente con la descripcion de 
    # la funcion
